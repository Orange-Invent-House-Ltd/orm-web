  ...
  if (showDatePicker) {
    return (
      <div className={`min-h-screen flex items-center justify-center transition-all duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'
        ...
        <div className={`w-full max-w-md rounded-xl shadow-lg p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <p className="text-neutral-400 text-[14px] font-[500] block text-left mb-6">You can only select 1 month at a time e.g. 2023-01-01 to 2023-01-31</p>
    ...
    <div className={`min-h-screen transition-all duration-300 sm:p-6 p-2 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'
        ...
        <div className="mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={() => setShowDatePicker(true)}
                className={`text-sm cursor-pointer ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}
              >
                {fromDate} to {toDate}
              </button>
      ...
      <main className="mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className={`text-3xl font-bold mb-2 mt-7 ${isDarkMode ? 'text-white' : 'text-gray-900'
              }`}>
              {currentBankData.bankName}
            </h2>
            <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {fromDate} to {toDate} • {analytics.transactionCount} transactions
            </p>
          </div>
          <BankLogoDisplay size="w-24 h-24" />
        </div>
        ...
        {hasFetchedData && transactions.length === 0 || noDataFound ? (
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <button
              onClick={() => setShowDatePicker(true)}
              className="mt-6 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium transition-colors"
            >
                    ...
                    <div key={index} className={`p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'
                      ...
                      <div className="flex justify-between items-start text-left">
                        ...
                        <div className="text-right">
                          <p className={`font-semibold ${transaction.creditAmount > 0
                            ? 'text-green-500'
                            : 'text-red-500'
                            }`}>
