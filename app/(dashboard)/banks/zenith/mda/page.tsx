...
export default function ZenithMdaPage() {
  const router = useRouter();
  const { setActiveBank } = useFinanceStore();
  const [page, setPage] = useState(1);
  const [size, setSize] = useState(20);
  const [seeAll, setSeeAll] = useState(false);
  const { data: zenith, isLoading, refetch } = useFetchZenithAggregatedBalance({ page, size, is_mda_account: true });
  ...
  const totalResults = zenith?.meta?.totalResults ?? 0;
    ...
    <div className="p-4 sm:p-8 min-h-screen" style={{ backgroundColor: "#0d1a11" }}>
      ...
      >
        <div className="flex items-center gap-2 text-xs mb-2" style={{ color: "rgba(255,255,255,0.4)" }}>
          ...
          <span>/</span>
          <Link href="/banks/zenith" className="hover:opacity-70" style={{ color: "#13ec5b" }}>{bankName}</Link>
          <span>/</span>
