...
export default function PtbBankPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useFetchPtbAggregatedBalance({ page, size, search });
    ...
    <div className="md:p-8 p-3 " style={{ backgroundColor: "#0d1a11" }}>
      ...
      >
        ...
        <div className="flex md:flex-row flex-col md:items-center items-start justify-between gap-4">
          ...
          <button
            className="sm:w-fit w-full flex justify-center items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all hover:opacity-80 mt-2"
            style={{
      ...
      >
        <button
          ...
          style={{ color: "#13ec5b", borderBottom: "2px solid #13ec5b" }}
        >
          All Accounts
        </button>
      ...
      {ptb?.meta?.totalResults > accounts.length && (
        <motion.button
          className="mt-4 w-fit flex items-center justify-center mx-auto p-6 bg-white/10 rounded-lg text-white font-bold text-sm mb-16"
          whileTap={{ scale: 0.98 }}
